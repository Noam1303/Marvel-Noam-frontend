...
const CharacterSingle = ({ user, setUser, Cookies, favoris, setFavoris }) => {
    ...
    const fetchData = async () => {
        const response = await axios.get(`https://site--test-backend--7g4fljlbl5js.code.run/comics/${id}`); // Requête pour récupérer les comics d'un personnage

        // Si l'utilisateur est connecté, récupérer ses favoris
        if (user.length > 0) {
            const idUser = user[1];
            const favoritesData = await axios.get(`https://site--test-backend--7g4fljlbl5js.code.run/user?id=${idUser}`);
