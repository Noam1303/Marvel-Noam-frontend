...
const Favoris = ({ user, setUser, Cookies, favoris, setFavoris }) => {
    ...
    const fetchData = async () => {
        ...
        const response = await axios.get("http://localhost:8000/favoris", {
            headers: {
        ...
        const articles = await Promise.all(
            newFavoris.map((favori) =>
                favori.iscomics && favori.iscomics === true
                    ? axios
                          .get(`http://localhost:8000/comic/${favori.articleId}`)
                          .then((res) => res.data) // Récupérer les détails si c'est un comic
                    : axios
                          .get(`http://localhost:8000/comics/${favori.articleId}`)
